...
3. 같은 행동을 하는 객체 - 클로저
   ...
   - 보통은 함수 안에서 사용된 로컬 변수(여기에서는 변수 `counter`)는 함수의 처리가 종료한 시점에 파기된다. 하지만 위의 예시에서는  `closure`함수로부터 되돌려진 `'익명 함수가 로컬 함수 counter를 계속 참조하고 있다'`는 이유로 `closure`함수의 종료 후에도 로컬 변수 `counter`는 계속해서 보관 유지된다.
